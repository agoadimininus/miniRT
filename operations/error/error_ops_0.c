...
/*   Updated: 2021/08/09 12:13:47 by cfico-vi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_error(char *code, int id)
{
	...
	fd = open("operations/error/errors.txt", O_RDONLY);
	if (fd == -1)
		error_exit(SYSERR);
...
void	error_exit(char *err_str)
{
	...
	{
		ft_putstr_fd("Error\nCRASHED - Oops, that was unexpected.\n",
			STDERR_FILENO);
		ft_putstr_fd("Error\nPlease get in contact if you can\n",
			STDERR_FILENO);
	}
